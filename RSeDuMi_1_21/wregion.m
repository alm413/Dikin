...
function [xscl,y,zscl,y0, w,relt, dxmdz,err, wr,center_count,ind,Dual_Imp] = ...
    wregion(L,Lden,Lsd,d,v,vfrm,A,DAt,dense, R,K,y,y0,b, pars, wr,center_count,ind)
%
...
% INITIAL CENTERING
% ----------------------------------------
if  strcmp(pars.version,'sedumi') && wr.delta > 0.0            % initial centering needed ?
...
% ----------------------------------------
if STOP ~= -1
    if strcmp(pars.version,'rsedumi') && ind==0
        pMode=4;
        center_count=center_count+1;
    end
    [dx,dy,dz,dy0, err,v_2,v_4] = sddir(L,Lden,Lsd,pv,...
        d,v,vfrm,A,DAt,dense, R,K,y,y0,b, pars,pMode);
    dxmdz = dx-dz;
    if (pars.alg ~= 0)
        pMode = 3;
    ...
    if strcmp(pars.version,'sedumi') && critval <= 0
        STOP = -1;
        ind=1;
    else
        % ----------------------------------------
        ...
        [t,wr,w,ind,Dual_Imp] = widelen(xc,zc,y0c, dx,dz,dy0,0, maxt,pars,K,v_2,v_4);
%         if ind==0
%            center_count=center_count+1;
%            wr=[];
%            w=[];
...
%                d,v,vfrm,A,DAt,dense, R,K,y,y0,b, pars,pMode);
%            pars.neigh=2;
%            [t,wr,w,ind] = widelen(xc,zc,y0c, dx,dz,dy0,0, maxt,pars,K,v_2,v_4);
%         end
        % ----------------------------------------
