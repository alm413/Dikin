...
% --------------------------------------------------
if ~isempty(K.q)
    %Ali: No idea what it is doing there
    At = qreshape(At,0,K);
    c = qreshape(c,0,K);
end
% ----------------------------------------
...
%Ali:outputs the size of each block including the nonnegative orthant and
%Lorentz cones
K = statsK(K);
...
%memory.
[i j s]=find(At);
%Ali:creates an K.N*m sparse matrix using the vectors i, j, and s 
At=sparse(i,j,s,K.N,m);
%Ali: calculates the sparsity of c 
if spars(c)<2/3
    [i j s]=find(c);
