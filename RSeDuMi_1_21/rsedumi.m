function [x,y,info] = rsedumi(A,b,c,K,pars)
...
% and print welcome.
% ----------------------------------------
%Ali: "pretransfo" casts the problem into the desired sedumi format;checks
%the vector and matrix size, transforms rotated SO cones to standard SO
%cones, searches for free variables, reshuffles the entries associated with
%the Lorentz blocks
[A,b,c,K,prep,origcoeff] = pretransfo(A,b,c,K,pars);
[N,m]=size(A);
%Ali: The problem is infeasible
if issparse(A)
    %The structural rank of the matrix
...
lponly = (K.l == length(c));
%Ali: Fills in defaults for missing fields in "pars" structure.
...
if strcmp(pars.version,'sedumi')
   ...
   my_fprintf(pars.fid,'Dikin-type primal-dual IPM for symmetric conic optimization, 2014. Ali Mohammad-Nezhad\n');
end
switch pars.alg
    case 0
...
% --------------------------------------------------
%Ali: need not be considered at this time
...
%Ali: Note that the merit function is divided by R.b0
...
if strcmp(pars.version,'sedumi')
    ...
    my_fprintf(pars.fid,' it :     b*y       gap    Imp     pinf     dinf    #Centering\n');
end
...
while STOP == 0
    ...
    % ------------------------------------------------------------
    y0Old = y0;
    [xscl,yNxt,zscl,y0Nxt, w,relt, dxmdz,err, wr,center_count,ind,Dual_Imp] = ...
        wregion(L,Lden,Lsd,...
        d,v,vfrm,A,DAt,dense, R,K,y,y0,b, pars, wr,center_count,ind);
    % ------------------------------------------------------------
    % Evaluate the computed step.
    % ------------------------------------------------------------
    %Ali: err.b=Ax-x0b-y0R.b which results in R.b = R.b + err.b / y0Nxt;
    %     err.c: -A'y+x0c+y0Rc-z
    %     err.db0: In the same way as above
     if strcmp(pars.version,'rsedumi') && (norm(err.b) >.001 || ind==3)
         STOP=-1;
         break;
     end
     if y0Nxt > 0
    ...
    rate = merit / meritOld;
   if strcmp(pars.version,'sedumi') && (rate >= 0.9999) && (wr.desc == 1)
%   ------------------------------------------------------------
...
%   ------------------------------------------------------------
    ...
    Dual_gap=norm(qreshape(v,1,K))^2/(K.l + 2 * length(K.q) + K.rLen + K.hLen);
    if strcmp(pars.version,'rsedumi') 
        x_prime = [sqrt(d.l).*v(1:K.l); asmDxq(d,v,K); psdscale(d,v,K,1)];
        x0_prime = x_prime(1);
        Ax_prime = Amul(A,dense,x_prime,0);
        Ay_prime = full(Amul(A,dense,y,1));
        pinf = norm(x0_prime*b-Ax_prime);
        z_prime = qreshape(Ay_prime-x0_prime*c,1,K);
        dinf = max(eigK(z_prime,K));
    end
    ...
    if strcmp(pars.version,'sedumi')
       ...
       my_fprintf(pars.fid,' %2.0f : %10.2E %2.2E %6.2f %6.4E %6.4E %2d  ',...
        iter,by/x0,Dual_gap,100*Dual_Imp,pinf,dinf,center_count);
    end
%     plot(iter,merit,'*-c');
    if pars.vplot == 1
